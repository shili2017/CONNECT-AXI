...
class AXI4StreamMasterBridge(implicit p: Parameters) extends Module {
  ...
  if (p(DEBUG_AXI4_BRIDGE)) {
    ...
    when(io.t_packet.fire) {
      printf("%d: [AXI4 Bridge-M%d] t_packet =%b\n", DebugTimer(), p(DEVICE_ID).U, io.t_packet.bits)
    }
  }
}

class AXI4StreamSlaveBridge(implicit p: Parameters) extends Module {
  val io = IO(new Bundle {
