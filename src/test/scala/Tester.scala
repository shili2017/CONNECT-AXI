...
class NetworkAXI4WrapperTester extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "pass AXI4 test" in {
    ...
    test(new AXI4Testbench(CLOCK_DIVIDER_FACTOR, TEST_LEN)(network_configs)).withAnnotations(annotation) { tb =>
      tb.clock.step()
      for (i <- 0 until network_configs.NUM_MASTER_DEVICES) {
...
class NetworkAXI4LiteWrapperTester extends AnyFlatSpec with ChiselScalatestTester {
  val network_configs = GetImplicitNetworkConfigs("AXI4-Lite")
  ...
  it should "pass AXI4-Lite test" in {
    ...
    test(new AXI4LiteTestbench(CLOCK_DIVIDER_FACTOR)(network_configs)).withAnnotations(annotation) { tb =>
      tb.clock.step()
      for (i <- 0 until network_configs.NUM_MASTER_DEVICES) {
        tb.io.start_write(i).poke(false)
        tb.io.start_read(i).poke(false)
        tb.io.target_addr(i).poke((i + network_configs.NUM_MASTER_DEVICES).U)
      }
      tb.clock.step(CLOCK_DIVIDER_FACTOR)
      tb.io.start_write(0).poke(true)
      tb.clock.step(CLOCK_DIVIDER_FACTOR)
...
class NetworkSimpleWrapperTester extends AnyFlatSpec with ChiselScalatestTester {
  val network_configs = GetImplicitNetworkConfigs("Simple")

  // Test simple wrapper with packet width = 72
  assert(network_configs.PACKET_WIDTH == 72)
  ...
  it should "pass simple test" in {
    ...
    test(new SimpleTestbench(CLOCK_DIVIDER_FACTOR)(network_configs)).withAnnotations(annotation) { tb =>
      val packet_0_2 = BigInt("e01234567812345678", 16)
      val packet_1_3 = BigInt("f1deadbeefdeadbeef", 16)

      tb.clock.step(CLOCK_DIVIDER_FACTOR)
      for (i <- 0 until network_configs.NUM_USER_SEND_PORTS) {
        tb.io.send(i).bits.poke(0.U)
        tb.io.send(i).valid.poke(false.B)
      }
      for (i <- 0 until network_configs.NUM_USER_RECV_PORTS) {
...
class NetworkAXI4StreamWrapperTester extends AnyFlatSpec with ChiselScalatestTester {
  val network_configs = GetImplicitNetworkConfigs("AXI4-Stream")
  ...
  it should "pass AXI4-Stream test" in {
    ...
    test(new AXI4StreamTestbench(CLOCK_DIVIDER_FACTOR, TEST_LEN)(network_configs)).withAnnotations(annotation) { tb =>
      tb.clock.step(CLOCK_DIVIDER_FACTOR)
      for (i <- 0 until network_configs.NUM_MASTER_DEVICES) {
        tb.io.start(i).poke(false)
        tb.io.target_dest(i).poke(((i + network_configs.NUM_MASTER_DEVICES / 2) % network_configs.NUM_MASTER_DEVICES).U)
      }
      tb.clock.step(CLOCK_DIVIDER_FACTOR)
